...
export(Texture) var dot_texture = preload('res://addons/GDCharts/graph-plot-white.png')
...
func _draw_labels():
  if current_show_label & LABELS_TO_SHOW.LEGEND_LABEL:
    ...
    for legend_label in current_point_color.keys():
      ...
      var label_position = position + LABEL_SPACE * Vector2(0.7, 0.4)
      ...
      position.x += 1.1 * max_x / nb_labels
